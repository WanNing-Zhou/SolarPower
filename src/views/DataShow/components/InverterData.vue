...
// 获取表单数据
const getAnalyseId = async (data?: InverterParams | any) => {
  if (!data) {
    const end = new Date()
    const start = new Date()
    start.setTime(start.getTime() - 3600 * 1000 * 24 * 14) // 最近三天数据
    data = {
...
watch(stationId, () => {
  // console.log('变化了')
  if (stationId.value) getAnalyseId()
